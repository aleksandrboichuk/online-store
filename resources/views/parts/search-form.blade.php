@if(preg_match("#^\/shop\/women\b#", request()->getRequestUri())
    ...
    || preg_match("#^\/shop\/boys\b#", request()->getRequestUri()))
        @if(preg_match("#^\/shop\/women\b#", request()->getRequestUri()))
            <form action="{{route('search', ['women'])}}" method="get" ><input type="text" name="q" placeholder="Пошук по товарам для жінок..." value=""/><button><i class="fa fa-search"></i></button></form>
        ...
        @elseif(preg_match("#^\/shop\/boys\b#", request()->getRequestUri()))
            <form action="{{route('search', ['boys'])}}" method="get" ><input type="text" name="q" placeholder="Пошук по товарам для хлопчиків..." value=""/> <button><i class="flaticon-search"></i></button></form>
